...
function SavedMovies({ savedMovies, setSavedMovies }) {
  ...
  useEffect(() => { // storage
    if (storage) {
      if (storage.savedSearch && storage.savedSearch.movies) setMovies(storage.savedSearch.movies);
      if (storage.savedSearch && storage.savedSearch.search) setInputValue(storage.savedSearch.search);
